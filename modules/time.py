...
def dtcalc():
    ...
    # check max timestep size from clastic sedimentation rate if clastic sedims are considered
    if v.comClasticSedi:
        dtClast = tstepsize(dtClast)

    # constrain timestep size from transport velocity (courant time) only if transport is considered
    if v.comTransport:
...
def tstepsize(timest):
    ...
    for node in range(v.nNode):
        ...
        if v.wDepth[node]-v.head[node] >= v.wMin:
            ...
            if settletime[node] <= 0.0:
                settletime[node] = 1000000.0
