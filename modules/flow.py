...
def flow():
    ...
    # function for instationary flow simulation
...
def vector():
    ...
    w = np.zeros(3, dtype=np.float64)
    # This function calculates flow vectors from a given potential field
    ...
    for n in range(v.nElem):
        ...
        if v.wDepth[v.inn[n][0]]-v.head[v.inn[n][0]] > 0.0:
            hTemp[0] = v.head[v.inn[n][0]]

        xTemp[1] = v.xCo[v.inn[n][1]]
        yTemp[1] = v.yCo[v.inn[n][1]]
        if v.wDepth[v.inn[n][1]]-v.head[v.inn[n][1]] > 0.0:
            hTemp[1] = v.head[v.inn[n][1]]

        xTemp[2] = v.xCo[v.inn[n][2]]
        yTemp[2] = v.yCo[v.inn[n][2]]
        if v.wDepth[v.inn[n][2]]-v.head[v.inn[n][2]] > 0.0:
            hTemp[2] = v.head[v.inn[n][2]]
...
def matsol_kinzelbach():
    # solution of system of equations from fe mesh adapted from Kinzelbach 1985 p. 290
