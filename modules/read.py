    ...
    try:
        ...
        for nod in range(v.nNode):
            ...
            if len(words) > 3:
                ...
                if v.readtypBC[nod] == 0:
                    v.headBC[nod] = float(words[4])
                if v.readtypBC[nod] == 3:
                    v.readQ[nod] = float(words[4])*31557600.0
            else:
...
def global_parameters():
    try:
        ...
        if len(words) > 1:
            if words[1] == 'y':
                v.comMassBalance = True
        line = fco.readline()
        words = str.split(line)
        if len(words) > 1:
            if words[1] == 'y':
                v.comClasticSedi = True
        line = fco.readline()
        words = str.split(line)
        if len(words) > 1:
            if words[1] == 'y':
        ...
        if len(words) > 1:
            if words[1] == 'y':
                v.comCompaction = True
        line = fco.readline()
        words = str.split(line)
        if len(words) > 1:
            if words[1] == 'y':
                v.comSubsidence = True
        fco.readline()
        line = fco.readline()
        words = str.split(line)
        if len(words) > 1:
...
#  read subsidence data
    ...
    if subsiType == 0:  # different subsidence for each node
        for node in range(v.nNode):
            line = fsp.readline()
            words = str.split(line)
            v.subsidence[int(words[0])] = float(words[1])
    elif subsiType == 1:  # same subsidence for all nodes
        v.subsidence = float(words[0])
    else:    # case default
        v.subsidence = 0.0
