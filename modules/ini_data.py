...
def initial_data():
    ...
    v.bw = v.nl * 2 + 1
    if v.bw > (2 * v.nCol + 1):
        print("bandwidth too large: ", v.bw)
        quit()
    ...
    for i in range(v.nNode):
        ...
        for j in range(6):
            for m in range(3):
                if v.nodElems[i][j] > 0:
                    cnTemp[n] = v.inn[v.nodElems[i][j]][m]
                n = n + 1
        for j in range(18):
            if cnTemp[j] == i:
                cnTemp[j] = 0
            for k in range(j - 1):
                if cnTemp[j] == cnTemp[k]:
                    cnTemp[j] = 0
        n = 0
        for j in range(18):
            if cnTemp[j] > 0:
                ...
                n = n + 1
    modules.compute.area_elem()
    modules.read.sedi_param()
    if v.comSubsidence:
        modules.read.subsidence()
    ...
    for node in range(v.nNode):
        if v.readtypBC[node] == 1 and v.headBC[node] > headMax:
            headMax = v.headBC[node]

    # set all nodes, which are not fixed head nodes to headmax
    for node in range(v.nNode):
        if v.readtypBC[node] != 1:
            v.headBC[node] = headMax
