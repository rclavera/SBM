...
#       average density of all sedim. types
    for m in range(v.nTotalMat):
      for t in range(v.jti):
        ...
        dumdens=dumdens+v.grid[nodewaderdepth()][t][m]*v.density[m]
...
#     coordenates x,#  print 'cTime:',v.cTime,'dt:',v.dt, 'totalTime:',v.totalTime, 'stepTime',v.stepTimey,z of punt 1
...
# n=u x b (producte escalar de dos vectors)
    ...
    if (n[i][2]<0.0):
      ...
      n[i][1]=-n[i][1]
      n[i][2]=-n[i][2]
    v.slope[te][i]=(math.acos(n[i][2]/(math.sqrt((n[i][0]**2.0)+(n[i][1]**2.0)+(n[i][2]**2.0)))))*57.29577951308232  #180/PI
  ...
  for i in range(v.nNode):
    if v.nodElems[i][6]==1:
      ...
      medNormal[2]=(n[v.nodElems[i][0]][2]+
                    #  print 'cTime:',v.cTime,'dt:',v.dt, 'totalTime:',v.totalTime, 'stepTime',v.stepTimen[v.nodElems[i][1]][2]+
                    n[v.nodElems[i][2]][2]+
                    n[v.nodElems[i][3]][2]+
                    n[v.nodElems[i][4]][2]+
                    n[v.nodElems[i][5]][2])/6

    v.slopeNode[te][i]=(math.acos(medNormal[2]/(math.sqrt((medNormal[0]**2.0)+(medNormal[1]**2.0)+(medNormal[2]**2.0)))))*57.29577951308232  #180/PI
...
def boundaryConditions():
  import random
# It checks the boun#  print 'cTime:',v.cTime,'dt:',v.dt, 'totalTime:',v.totalTime, 'stepTime',v.stepTimedary conditions to move them when sealevel changes
...
# if a node is over sea level and have BC look for the nearest nodes in the coastline
      if (v.wDepth[node]<=0.0):
        ...
        for j in range(v.nClsMat):
          if(v.clastQ[node][j]>0.0): tempc=tempc+1
            ...
            if dista[i]>0.0:
              ...
              for ii in range(6):
                if (v.connectNodes[i][ii]==0.0):
                  ...
                  if (v.wDepth[v.connectNodes[i][ii]]-v.head[v.connectNodes[i][ii]]<=0.0): canvi=canvi+1
              if (canvi>=3):dista[i]=0.0
          for i in range (v.nNode):
            if dista[i]<=0.0: dista[i]=1000000000.0
...
def massBalance(material,cc):

#       subroutine to correct mass balance due to hyperbolic dominance in
...
#       Calculation of outflow mass in a dt	 
  ...
  if (sediOutFlow<0.0): sediOutFlow=-sediOutFlow
