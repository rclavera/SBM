...
def transport():
    # Function for transport simulation in combination
...
#       define corresponding boundary conditions to transport
    ...
    # for node in range (v.nNode):
        ...
        for node in range(v.nNode):
            if cc[node] < 0.0 or v.wDepth[node] <= 0.0:
                cc[node] = 0.0

#     Calculate mass balance
        if v.comMassBalance:
            com.mass_balance(material, cc)
...
def setboundary(qm, material):
    ...
    for i in range(v.nNode):
        if v.conClast[i][material] < 0.0:
            v.conClast[i][material] = 0.0
        if v.clastQ[i][material] > 0.0 and v.wDepth[i] > 0:
            qm[i] = v.clastQ[i][material]/v.wDepth[i]
